...
int checkRobotSensorFrontRightAllWalls(struct Robot * robot, struct Wall_collection * head) {
    ...
    {
        ptr = head_store;
        xDir = round(robotCentreX+(ROBOT_WIDTH/2-2)*cos((robot->angle)*PI/180)-(-ROBOT_HEIGHT/2-SENSOR_VISION+sensorSensitivityLength*i)*sin((robot->angle+15)*PI/180));
        yDir = round(robotCentreY+(ROBOT_WIDTH/2-2)*sin((robot->angle)*PI/180)+(-ROBOT_HEIGHT/2-SENSOR_VISION+sensorSensitivityLength*i)*cos((robot->angle+15)*PI/180));
        xTL = (int) xDir;
        yTL = (int) yDir;
        hit = 0;
...
int checkRobotSensorFrontLeftAllWalls(struct Robot * robot, struct Wall_collection * head) {
    ...
    {
        ptr = head_store;
        xDir = round(robotCentreX+(-ROBOT_WIDTH/2)*cos((robot->angle)*PI/180)-(-ROBOT_HEIGHT/2-SENSOR_VISION+sensorSensitivityLength*i)*sin((robot->angle-15)*PI/180));
        yDir = round(robotCentreY+(-ROBOT_WIDTH/2)*sin((robot->angle)*PI/180)+(-ROBOT_HEIGHT/2-SENSOR_VISION+sensorSensitivityLength*i)*cos((robot->angle-15)*PI/180));
        xTL = (int) xDir;
        yTL = (int) yDir;
        hit = 0;
...
int checkRobotSensorRightAllWalls(struct Robot * robot, struct Wall_collection * head) {
    ...
    {
        ptr = head_store;
        xDir = round(robotCentreX+(ROBOT_WIDTH/2-2)*cos((robot->angle)*PI/180)-(-ROBOT_HEIGHT/2-SENSOR_VISION+sensorSensitivityLength*i)*sin((robot->angle+90)*PI/180));
        yDir = round(robotCentreY+(ROBOT_WIDTH/2-2)*sin((robot->angle)*PI/180)+(-ROBOT_HEIGHT/2-SENSOR_VISION+sensorSensitivityLength*i)*cos((robot->angle+90)*PI/180));
        xTL = (int) xDir;
        yTL = (int) yDir;
        hit = 0;
...
void robotUpdate(struct SDL_Renderer * renderer, struct Robot * robot){
    ...
    */
    ...
    for (i = 0; i < 5; i++)
    {
        xDir = round(robotCentreX+(ROBOT_WIDTH/2-2)*cos((robot->angle)*PI/180)-(-ROBOT_HEIGHT/2-SENSOR_VISION+sensor_sensitivity*i)*sin((robot->angle+15)*PI/180));
        yDir = round(robotCentreY+(ROBOT_WIDTH/2-2)*sin((robot->angle)*PI/180)+(-ROBOT_HEIGHT/2-SENSOR_VISION+sensor_sensitivity*i)*cos((robot->angle+15)*PI/180));
        xTL = (int) xDir;
        yTL = (int) yDir;
    ...
    for (i = 0; i < 5; i++)
    {
        xDir = round(robotCentreX+(-ROBOT_WIDTH/2)*cos((robot->angle)*PI/180)-(-ROBOT_HEIGHT/2-SENSOR_VISION+sensor_sensitivity*i)*sin((robot->angle-15)*PI/180));
        yDir = round(robotCentreY+(-ROBOT_WIDTH/2)*sin((robot->angle)*PI/180)+(-ROBOT_HEIGHT/2-SENSOR_VISION+sensor_sensitivity*i)*cos((robot->angle-15)*PI/180));
        xTL = (int) xDir;
        yTL = (int) yDir;
    ...
    for (i = 0; i < 5; i++)
    {
        xDir = round(robotCentreX+(ROBOT_WIDTH/2-2)*cos((robot->angle)*PI/180)-(-ROBOT_HEIGHT/2-SENSOR_VISION+sensor_sensitivity*i)*sin((robot->angle+90)*PI/180));
        yDir = round(robotCentreY+(ROBOT_WIDTH/2-2)*sin((robot->angle)*PI/180)+(-ROBOT_HEIGHT/2-SENSOR_VISION+sensor_sensitivity*i)*cos((robot->angle+90)*PI/180));
        xTL = (int) xDir;
        yTL = (int) yDir;
...
void robotAutoMotorMove(struct Robot * robot, int front_left_sensor, int front_right_sensor, int right_sensor, int *counter) {

        if ((front_left_sensor == 0) && (front_right_sensor == 0)&&(right_sensor !=0)) {
            if (robot->currentSpeed<=2 && robot->currentSpeed>= 0 )
                robot->direction = UP;
                *counter = 0;
        }
        if ((front_left_sensor>=1) && (front_right_sensor >=1 )&&(right_sensor >= 1) ){
            *counter += 1;
            if ((*counter >= 1)&&(*counter <2)){
                if (robot->currentSpeed >= 0)
                    robot->direction = DOWN;
                if (robot->currentSpeed >= -3 && robot->currentSpeed<=0 ){
                    robot->direction = UP;
                }
            }
            else{
...
//            *counter = 0;
//        }
        if ((front_left_sensor>=1) && (front_right_sensor >=1 )&&(right_sensor == 0) ){
            robot->direction = LEFT;
            *counter = 0;
        }
        if ((front_left_sensor>=1) && (front_right_sensor == 0)&&(right_sensor == 0) ){
            robot->direction = RIGHT;
            *counter = 0;
        }
        if ((front_left_sensor>=1) && (front_right_sensor == 0)&&(right_sensor >= 1) ){
            robot->direction = RIGHT;
            *counter = 0;
        }
        if ((front_left_sensor==0) && (front_right_sensor >=1 )&&(right_sensor == 0) ){
            robot->direction = LEFT;
            *counter = 0;
        }
        if ((front_left_sensor==0) && (front_right_sensor >=1 )&&(right_sensor >= 1) ){
            robot->direction = LEFT;
            *counter = 0;
        }
        if ((front_right_sensor==0) && (front_left_sensor == 0)&&(right_sensor==0)){
            *counter += 1;
            if (*counter < 15){
                robot->direction = RIGHT;
            }
        ...
        if ((front_right_sensor == 1)&&(right_sensor == 1)){
            robot->direction = LEFT;
            *counter = 0;
        }
//        if ((front_left_sensor==0)&&(front_right_sensor==0)&&(right_sensor>3)){
//            robot->direction = LEFT;
//            *counter = 0;
//        }
